...
const getTimeInfo = (state, duration) => {
  if(state.building) {
    ...
    return `${remaining < -20000 ? 
        chalk`${getHumanReadableTime(duration)} {redBright.bold overdue}` : 
        remaining < 20000 ? 
            'any moment' : 
...
const printer = () => {
  ...
  return (state) => {
    ...
    if(state.building) {
      ...
      spinner.succeed(chalk`{blue.bold.underline ${state.name}} ${getTimeInfo(state, duration)}`);
    } else {
      spinner.fail(chalk`{blue.bold.underline ${state.name}} ${getTimeInfo(state, duration)}`);
    }
  }
};
